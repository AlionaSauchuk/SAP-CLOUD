sap.ui.define([
	...
	var oSandbox = sinon.sandbox.create(),
		aBooks, // The array that holds the cached book data
	...
	/**
	 * Looks for a book with a given book name and returns its index in the book array.
	 * @param {string} sBookName - the book name to look for.
	 * @returns {int} index of that book in the array, or -1 if the book was not found.
	 */
	function findBookIndex(sBookName) {
		for (var i = 0; i < aBooks.length; i++) {
			if (aBooks[i].BookName === sBookName) {
				return i;
			}
		}
		return -1;
	...
	/**
	 * Retrieves any book data from a given http request body.
	 * @param {string} sBody - the http request body.
	 * @returns {Object} the parsed book data.
	 */
	function getBookDataFromRequestBody(sBody) {
		var aMatches = sBody.match(/({.+})/);
		if (!Array.isArray(aMatches) || aMatches.length !== 2) {
			throw new Error("Could not find any book data in " + sBody);
		}
		return JSON.parse(aMatches[1]);
	}

	/**
	 * Retrieves a book name from a given request URL.
	 * @param {string} sUrl - the request URL.
	 * @returns {string} the book name.
	 */
	function getBookKeyFromUrl(sUrl) {
		var aMatches = sUrl.match(/People\('(.*)'\)/);
		if (!Array.isArray(aMatches) || aMatches.length !== 2) {
			throw new Error("Could not find a book key in " + sUrl);
		}
		return aMatches[1];
	}

	/**
	 * Checks if a given BookName is unique or already used
	 * @param {string} sBookName - the BookName to be checked
	 * @returns {boolean} True if the BookName is unique (not used), false otherwise
	 */
	function isUnique(sBookName) {
		return findBookIndex(sBookName) < 0;
	}
	...
	function duplicateKeyError(sKey) {
		return JSON.stringify({
			error : {
				code : "409",
				message : "There is already a book with book name '" + sKey + "'.",
				target : "BookName"
			}
		});
	}
	...
	function invalidKeyError(sKey) {
		return JSON.stringify({
			error : {
				code : "404",
				message : "There is no book with book name '" + sKey + "'.",
				target : "BookName"
			}
		});
	}
	...
	function applySkipTop(oXhr, aResultSet) {
		var iSkip,
			iTop,
			aReducedBooks = [].concat(aResultSet),
	...
	function applySort(oXhr, aResultSet) {
		var sFieldName,
			sDirection,
			aSortedBooks = [].concat(aResultSet), // work with a copy
		...
		if (!Array.isArray(aMatches) || aMatches.length < 2) {
			...
			aSortedBooks.sort(function (a, b) {
	...
	function applyFilter(oXhr, aResultSet) {
		var sFieldName,
			sQuery,
			aFilteredBooks = [].concat(aResultSet), // work with a copy
		...
		if (Array.isArray(aMatches) && aMatches.length >= 3) {
			...
			aFilteredBooks = aBooks.filter(function (oBook) {
				return oBook.LastName.indexOf(sQuery) !== -1;
			});
		}
	...
	/**
	 * Handles GET requests for a pure book count and returns a fitting response.
	 * @returns {Array} an array with the response information needed by Sinon's respond() function
	 */
	function handleGetCountRequests() {
		return getSuccessResponse(aBooks.length.toString());
	}

	/**
	 * Handles GET requests for book data and returns a fitting response.
	 * @param {Object} oXhr - the Sinon fake XMLHttpRequest
	 * @param {boolean} bCount - true if the request should include a counter
	 * @returns {Array} an array with the response information needed by Sinon's respond() function
	 */
	function handleGetBookRequests(oXhr, bCount) {
		var iCount,
			sKey,
			iIndex,
			sCount = "",
		...
		// Check if an individual book or a book range is requested
		try {
			sKey = getBookKeyFromUrl(oXhr.url); // If this throws an error, then a book range was requested

			iIndex = findBookIndex(sKey);
			if (iIndex > -1) {
				sResponseBody = '{"@odata.context": "' + getBaseUrl(oXhr.url) +
					'$metadata#People(Age,FirstName,LastName,BookName)/$entity",' +
					JSON.stringify(aBooks[iIndex]).slice(1);
			...
			// If getBookKeyFromUrl throws an error, then a book range was requested
			// Get the data filtered, sorted and reduced according to skip + top
			aResult = applyFilter(oXhr, aBooks);
			...
			sResponseBody = '{"@odata.context": "' + getBaseUrl(oXhr.url) +
				'$metadata#People(Age,FirstName,LastName,BookName)",' +
				sCount +
	...
	/**
	 * Handles PATCH requests for books and returns a fitting response.
	 * Changes the book data according to the request.
	 * @param {Object} oXhr - the Sinon fake XMLHttpRequest
	 * @returns {Array} an array with the response information needed by Sinon's respond() function
	 */
	function handlePatchBookRequests(oXhr) {
		var sKey,
			oBook,
			oChanges,
			sResponseBody;
		...
		sKey = getBookKeyFromUrl(oXhr.url);

		// Get the list of changes
		oChanges = getBookDataFromRequestBody(oXhr.requestBody);

		// Check if the BookName is changed to a duplicate.
		// If the BookName is "changed" to its current value, that is not an error.
		if (oChanges.hasOwnProperty("BookName") && oChanges.BookName !== sKey && !isUnique(oChanges.BookName)) {
			// Error
			sResponseBody = duplicateKeyError(oChanges.BookName);
			return [
				400,
				{
					"Content-Type" : "application/json; charset=utf-8"
			...
			oBook = aBooks[findBookIndex(sKey)];
			for (var sFieldName in oChanges) {
				if (oChanges.hasOwnProperty(sFieldName)) {
					oBook[sFieldName] = oChanges[sFieldName];
				}
			}
	...
	/**
	 * Handles DELETE requests for books and returns a fitting response.
	 * Deletes the book according to the request.
	 * @param {Object} oXhr - the Sinon fake XMLHttpRequest
	 * @returns {Array} an array with the response information needed by Sinon's respond() function
	 */
	function handleDeleteBookRequests(oXhr) {
		var sKey;

		sKey = getBookKeyFromUrl(oXhr.url);
		aBooks.splice(findBookIndex(sKey), 1);
	...
	/**
	 * Handles POST requests for books and returns a fitting response.
	 * Creates a new book according to the request.
	 * Does NOT check for duplicate book names because that is how the live service behaves.
	 * @param {Object} oXhr - the Sinon fake XMLHttpRequest
	 * @returns {Array} an array with the response information needed by Sinon's respond() function
	 */
	function handlePostBookRequests(oXhr) {
		var oBook,
			sResponseBody;

		oBook = getBookDataFromRequestBody(oXhr.requestBody);

		// Check if that book already exists
		if (isUnique(oBook.BookName)) {
			aBooks.push(oBook);

			sResponseBody = '{"@odata.context": "' + getBaseUrl(oXhr.url) + '$metadata#People/$entity",';
			sResponseBody += JSON.stringify(oBook).slice(1);
			...
			// Error
			sResponseBody = duplicateKeyError(oBook.BookName);
			return [
				400,
				{
					"Content-Type" : "application/json; charset=utf-8"
	 ...
	 * Reloads the base book data from file.
	 * Does NOT check for duplicate book names because that is how the live service behaves.
	 * @returns {Array} an array with the response information needed by Sinon's respond() function
	 */
		...
		switch (oXhr.method) {
			case "GET":
				if (/\$metadata/.test(oXhr.url)) {
					...
					aResponse = handleGetBookRequests(oXhr, /\$count=true/.test(oXhr.url));
				}
				break;
			case "PATCH":
				if (/People/.test(oXhr.url)) {
					aResponse = handlePatchBookRequests(oXhr);
				}
				break;
			case "POST":
				if (/People/.test(oXhr.url)) {
					aResponse = handlePostBookRequests(oXhr);
			...
			case "DELETE":
				if (/People/.test(oXhr.url)) {
					aResponse = handleDeleteBookRequests(oXhr);
				}
				break;
			default:
				break;
